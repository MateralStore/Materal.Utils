...
namespace Materal.Utils.Network.WebService;
...
public class WebServiceHelper : IWebServiceHelper
{
    ...
    {
        if (xmlNode is null) return null;
        if (!propertyType.IsClass || propertyType == typeof(string)) return ConvertHelper.ConvertTo(xmlNode.InnerText, propertyType);
