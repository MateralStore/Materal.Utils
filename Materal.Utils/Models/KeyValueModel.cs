...
public static class KeyValueModelExtension
{
    ...
    public static List<KeyValueModel> ToKeyValueModel(this Dictionary<string, string> dictionary)
        => [.. dictionary.Select(item => new KeyValueModel(item.Key, item.Value))];
    /// <summary>
    /// 转换为键值对模型
    /// </summary>
    /// <param name="dictionary"></param>
    ...
    public static List<KeyValueModel<TKey, TValue>> ToKeyValueModel<TKey, TValue>(this Dictionary<TKey, TValue> dictionary)
        where TKey : notnull
        => [.. dictionary.Select(item => new KeyValueModel<TKey, TValue>(item.Key, item.Value))];
    /// <summary>
    /// 获得所有识别码
    /// </summary>
    /// <typeparam name="TEnum"></typeparam>
        ...
        where TEnum : struct, Enum
    {
        ...
        List<KeyValueModel<TEnum>> result = [.. allCodeList.Select(item => new KeyValueModel<TEnum>((TEnum)item, item.GetDescription()))];
        return result;
    }
}
