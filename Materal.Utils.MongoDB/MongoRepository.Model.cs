    ...
    where T : class, new()
{
    ...
    private static SortDefinition<T>? GetSortDefinition(Expression<Func<T, object>> sortExpression)
        => GetSortDefinition(sortExpression, SortOrder.Ascending);
    /// <summary>
    /// 获取排序
    /// </summary>
    /// <param name="sortExpression"></param>
    ...
    /// <returns></returns>
    private static SortDefinition<T>? GetSortDefinition(Expression<Func<T, object>> sortExpression, SortOrder sortOrder) => sortOrder switch
    {
        SortOrder.Ascending => Builders<T>.Sort.Ascending(sortExpression),
        SortOrder.Descending => Builders<T>.Sort.Descending(sortExpression),
        _ => null
    };
    /// <summary>
    /// 获取查询选项
    ...
    /// <param name="sortOrder"></param>
    /// <returns></returns>
    private static FindOptions<T, T> GetFindOptions(Expression<Func<T, object>> sortExpression, SortOrder sortOrder) => new() { Sort = GetSortDefinition(sortExpression, sortOrder) };
    #endregion
}
